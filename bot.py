...
async def _(event):
    ...
    if event.user_joined or event.user_added:
        ...
        if x is True:
            ...
            butt = [Button.url("Canal", url=f"https://t.me/{channel}"), Button.inline("Desmuteame", data=f"unmute_{user.id}")]
...
async def mute_on_msg(event):
    ...
    if x is False:
        ...
        try:
            ...
            print(str(e))
            return
        await event.reply(f"ammmm {nm}, parece que no te has unido a nuestro canal. unete a @{channel} y presiona el boton para hablar!", buttons=[[Button.url("Canal", url=f"https://t.me/{channel}")], [Button.inline("Desmuteame", data=f"unmute_{event.sender_id}")]])
...
async def _(event):
    ...
    if uid == event.sender_id:
        ...
        if x is False:
            await event.answer(f"No te has unido a @{channel} !", cache_time=0, alert=True)
        elif x is True:
            try:
        ...
        await event.answer("tu que quieres!", cache_time=0, alert=True)

@BotzHub.on(events.NewMessage(pattern="/start"))
async def strt(event):
    await event.reply(f"Este bot es especial solo para @{channel}!", buttons=[Button.url("Canal", url=f"https://t.me/{channel}"), Button.url("Grupo", url="https://t.me/setzolandv2")])
